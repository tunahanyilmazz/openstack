...
# Check if the Keystone database exists
echo "Checking if Keystone database exists..."
DB_EXISTS=$(sudo mysql -u root -p$DB_ROOT_PASS -e "SHOW DATABASES LIKE 'keystone';" | grep keystone)

if [ "$DB_EXISTS" == "keystone" ]; then
    echo "Keystone database already exists. Deleting it."
    sudo mysql -u root -p$DB_ROOT_PASS -e "DROP DATABASE keystone;"
fi
...
echo "Verifying Keystone database population..."
TABLES=$(sudo mysql -u root -p$DB_ROOT_PASS -e "USE keystone; SHOW TABLES;" | wc -l)
if [ $TABLES -le 1 ]; then
    echo "Error: Keystone database is empty. Check the logs for details."
    sudo tail -n 20 /var/log/keystone/keystone.log
    exit 1
else
    echo "Keystone database has been populated successfully."
fi
